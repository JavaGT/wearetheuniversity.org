...
const page_files = await fsp.glob('./source/pages/**/*.md')
const page_template = pug.compileFile('./source/templates/page.pug')

for await (const file of page_files) {
    // read the file
    ...
    const html = page_template({ attributes, content, title: attributes.title })

    // write the file
    const new_filepath = path.join('./docs', attributes.slug, 'index.html')
    await fsp.mkdir(path.dirname(new_filepath), { recursive: true })
    await fsp.writeFile(new_filepath, html)
}
