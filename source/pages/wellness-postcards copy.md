...
<h1 id="js-warning">Javascript is required to use this page</h1>
<script>
...
<script type="module">
    ...
    data_structure.forEach((data) => {
        ...
        document.getElementById('inputs').appendChild(fieldset);
    ...
    function sendDataToForms(){
        const submissionID = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
        const privateData = {
            submissionID: submissionID,
            email: '',
            name: '',
            permission: document.querySelector('input[name="Permission"]:checked').value,
            prompt: document.querySelector('select[name="Prompt"]').value,
            message: document.querySelector('textarea[name="Message"]').value,
            signedBy: document.querySelector('input[name="Signed by"]').value,
            font: document.querySelector('input[name="Font"]:checked').value,
            fontSize: document.querySelector('input[name="Font Size"]').value,
            image: document.querySelector('input[name="Image"]:checked').value
        }


        if (privateData.permission === 'yes' || privateData.permission === 'yes-anon') {
            const publicData = {
                submissionID: submissionID,
                prompt: document.querySelector('select[name="Prompt"]').value,
                message: document.querySelector('textarea[name="Message"]').value,
            ...
            if (privateData.permission === 'yes-anon') {
                publicData.signedBy = 'Anonymous';
            }
        }
